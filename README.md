...
| **[leancrypto](https://github.com/smuellerDD/leancrypto)**<br><sub>PQC for bare-metal and Linux kernel</sub> | Five defects reported and fixed. ASN.1 memory leaks on error paths across six key-parsing modules (ML-DSA, ML-DSA+Ed25519/Ed448, Ed25519, Ed448, SLH-DSA) ([`6581568`](https://github.com/smuellerDD/leancrypto/commit/6581568a992b7e5e1799323ed4714fe99e1b19a6)); `pathLenConstraint` not enforced per RFC 5280 §4.2.1.9, so a chain could exceed an intermediate CA's authorised depth ([`12cc2aea`](https://github.com/smuellerDD/leancrypto/commit/12cc2aeafdb257972ee3421e6761df8ac4cf947c)); a GCM buffer overflow where an over-long caller-supplied tag length writes zeros past the tag buffer ([`4e46730`](https://github.com/smuellerDD/leancrypto/commit/4e4673097660b3832bc429d5d39a02e842f5a97c)); and a pairwise-consistency-test return check that did not detect failure, fixed at the root in `lc_memcmp_secure` and therefore covering HQC as well as the ML-KEM path reported ([`fa8778a`](https://github.com/smuellerDD/leancrypto/commit/fa8778ae919ce58fd20a2dade1f9220b74a5a0f2)). A fifth, an XDRBG instance marking itself initially seeded after a rejected over-long personalization string ([`b737e9d`](https://github.com/smuellerDD/leancrypto/commit/b737e9d7ab628a599456313b1baa7aa269b18cb7)), the maintainer classed as a correctness bug rather than a security issue, and it is listed here on that basis | All five fixed upstream, each crediting me in the commit trailer. The path-length fix shipped with a new test-certificate matrix |
...
| **[simage](https://github.com/coin3d/simage)**<br><sub>image I/O for Coin3D / Open Inventor</sub> | Two heap buffer-overflow **writes** in the SGI RGB loader, both reachable from `simage_read_image()` (CWE-787, CWE-129, CWE-195), affected `<= 1.8.4`. The header's component count is read from the file and reaches 65535 while indexing a fixed four-entry array, so it wrote live `malloc()` pointers past the end of the struct and the close path then `free()`d across the same out-of-bounds range. Separately, the row-length field is signed and the existing test was an upper bound only, so a negative value skipped the buffer regrow and reached `fread()`, whose `size_t` parameter turns `-1` into a huge count. Reported with two Python reproducers and a release-build control | Fixed two days after the report in [`b3b7452`](https://github.com/coin3d/simage/commit/b3b74525) — *“Validate SGI RGB header fields before they index or size buffers”* — which closes both writes. Credited by name in the project's [THANKS file](https://github.com/coin3d/simage/commit/85f71ee956ba6c809a720dcf971e83032485ec4e) |
